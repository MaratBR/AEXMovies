...
public class AuthTest : AuthTestBase
{
    ...
    public async Task RefreshToken()
    {
        ...
        var response = await Client.PostAsync("/api/v1/auth/refresh",
            JsonContent.Create(new { refreshToken = result.RefreshToken }));
        response.EnsureSuccessStatusCode();
    }
    ...
    public async Task Register()
    {
        var response = await Client.PostAsync("/api/v1/auth/register",
            JsonContent.Create(new { userName = "Test", password = "Password!@#3423" }));
        response.EnsureSuccessStatusCode();

        response = await Client.PostAsync("/api/v1/auth/register",
            JsonContent.Create(new { userName = "Test", password = "Password!@#3423" }));
