...
public class EfRepository<TModel> : IRepository<TModel> where TModel : class
{
    ...
    public Task<List<TModel>> FindMany(Expression<Func<TModel, bool>>? filter = null, int? skip = null,
        int? take = null)
    {
        return Query(filter, skip, take).ToListAsync();
    }

    public Task<List<TProjection>> FindMany<TProjection>(Expression<Func<TModel, bool>>? filter = null,
    ...
    {
        if (_context.Entry(record).State == EntityState.Detached) _dbSet.Add(record);
        await _context.SaveChangesAsync();
    }
